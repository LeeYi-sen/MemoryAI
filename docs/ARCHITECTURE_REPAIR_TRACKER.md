...
| AR-002 | P0 | OPEN | Memory seed uses obsolete opcodes | `Kernel/current-required-structures.json; Kernel/src/*` | Migrate cognition_concurrency_set, cognition_stats, mesh_route_cognition to current physical ABI without moving policy into Kernel. | - | - |
...
| AR-007 | P0 | OPEN | Semantic Memories miss event.emit capability | `Kernel/current-required-structures.json` | Add required capabilities and permanent capability audit. | - | - |
...
| AR-013 | P0 | OPEN | Kernel Memory schema drops input_pattern/output_effect/executable | `Kernel/src/kernel.go; persistence tests` | Make required executable-structure fields round-trip safely. | - | - |
