...
| AR-006 | P1 | DONE | v31 semantic event graph is disconnected | `Kernel/current-required-structures.json` | Connect live Experience/semantic events without hard-coded language semantics in Kernel. | Entry 035 | live semantic event chain PASS; architecture audit PASS |
...
| AR-008 | P1 | DONE | Natural-language reply path is incomplete | `Current seed; daemon response boundary` | Close cognition -> expression frame -> learned surface rendering -> reply result. | Entry 035 | Chinese live reply x20 PASS; full Go test PASS |
| AR-009 | P1 | DONE | Initial Memory has no Chinese grounding basis | `Current seed` | Provide Memory-owned Chinese bootstrap grounding/evidence without Kernel language tables. | Entry 035 | Memory-owned Chinese bootstrap contract PASS; production Go literal scan clean |
| AR-010 | P1 | DONE | Executable mutation exists but recombination/fission is absent | `Current seed; generic VM program mutation primitives` | Implement Memory-owned structure recombination/fission using generic program splice primitives. | Entry 036 | recombination/fission integration PASS; focused race PASS |
| AR-011 | P1 | DONE | Memory body fusion decision chain is absent | `Current seed; space primitives` | Add Memory-owned discover/evaluate/decide/execute body-fusion flow. | Entry 036 | four-stage fusion contract PASS; live fusion integration PASS |
| AR-012 | P1 | DONE | Kernel space_merge contains reconciliation policy | `Kernel/src/kernel.go` | Reduce merge to physical primitives; preserve conflicts for Memory-owned resolution. | Entry 036 | divergent-ID physical preservation test PASS; no Kernel reconcile call |
...
| AR-020 | P1 | DONE | Kernel recognizes cog.surface/cog.input for trace policy | `Kernel/src/kernel.go` | Remove cognition-label interpretation from Kernel tracing. | Entry 038 | tag-agnostic trace runtime PASS; permanent cognitive namespace audit PASS |
| AR-021 | P2 | DONE | Physical storage metadata uses cog.storage namespace | `Kernel/src/kernel.go; current seed if referenced` | Rename to physical storage namespace with compatibility migration. | Entry 038 | physical.storage active namespace + one-way legacy migration/restart PASS |
...
- 2026-09-18 / Entry 038: Batch G DONE. AR-020/021 closed. Kernel execution tracing is now tag-agnostic physical telemetry, active storage metadata uses physical.storage.* only, and legacy cog.storage.* tags are migrated one-way inside Memory.mem. Frozen repair tracker is now 29/29 DONE.
