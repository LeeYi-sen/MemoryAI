...
### Entry 012 — Memory Growth Core: native `Memory.mem` persistence binding
- **Base HEAD:** `a16eca045dea12fb38bd8ba1c8a7985d21181b48`.
- **Architecture gap addressed:** Entries 009-011 had correct Memory-owned growth primitives, but their ledgers were still process-memory state. That would violate the final deployment model if growth state required a second database or sidecar.
- **Changed:** added `Kernel/src/memory_growth_persistence.go` and `Kernel/src/memory_growth_persistence_test.go`. Experience order/data/validation history, candidate Structures, pending Structure validation history, and validated Structures are serialized into one reserved ordinary Memory record `__memoryai.growth.core` inside the existing `memory.mem` body. The existing full-body persistence and in-body mutation journal therefore persist the growth state; no `experience.db`, `structures.db`, JSON state file, WAL/delta/journal sidecar or third runtime artifact is introduced.
- **Recovery:** `LoadMemoryGrowthState` reconstructs the three Memory-owned growth ledgers from that Memory record, including parent/child lineage, validation history, pending promotion counters and validated Structure state. `PersistMemoryGrowthState` marks only that Memory record dirty/new so existing persistence machinery owns the physical durability path.
- **Boundary:** the new file is source/build code only. It is not a runtime deployment dependency. Final runtime remains exactly two artifacts: the hard-coded startup program and `memory.mem`.
- **Validation:** added restart persistence coverage plus an explicit directory check proving no runtime sidecar/artifact is created. Full repository Gate remains unreported because GitHub Actions runners remain unavailable.
- **Next:** wire this Memory-native state into the live execution path, then replace exact static-pattern validation with actual executable Structure invocation -> predicted outcome -> real outcome -> new Experience feedback.
