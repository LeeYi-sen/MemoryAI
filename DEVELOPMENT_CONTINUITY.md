...
Current overlay chain includes Kernel ABI cleanup, Mesh binding, speculative boundaries, lazy speculation, scale/remote-memory/persistence/shard/store/fabric/owner-lock fixes, remote physical body transaction fixes, and fixed-score primitive removal.
...
1. **Mesh deferred journal is currently an unbounded in-memory `[]MeshRequest`.** If Sovereign authority is unavailable for a long time, repeated shared proposals can grow RAM without bound. Replace with bounded physical spool/backpressure and recovery semantics. Kernel should not accumulate unlimited network-failure state in RAM.
2. **Remote `remote_space_create/put/upsert/delete` durability ACK ordering is not fully source-proven yet.** Verify server success is returned only after durable target persistence; for any move/migration path, source deletion must occur only after target durability ACK.
3. Re-run full recovery chain and static package closure after the next overlay changes: restore -> gofmt -> vet -> test -> race when executable runner is available.
4. Continue scanning Kernel production runtime for residual hard-coded cognitive concepts/ranking/vector semantics.
