...
### Entry 023
- Base HEAD: `4c362da0ccaaed1652df511092802ab9ae804e07`.
- Added transient Sovereign Memory Mesh evidence views for four Memory-native evidence classes: `Experience`, `Belief`, validated/contested `Context Split`, and `Grounded Action Instance`. The requester receives a value envelope only; no requester-side Memory record, cache, sidecar, import, `upsert` or persistence path is created.
- Every evidence read performs a fresh existing `shared_fetch` authorization round with the Sovereign and a fresh origin-node access. Disconnect/unreachable origin therefore makes the evidence unavailable immediately; no stale local copy can substitute for the remote Memory. Reconnection can make it readable again through a new grant.
- Provenance is carried with every transient read: evidence kind/id, requester node, origin node/endpoint, physical backing Memory id/revision, read timestamp, Sovereign grant nonce/expiry, and SHA-256 payload digest. Kernel does not assign truth/confidence/priority to the evidence.
- `Experience` remains physically inside `__memoryai.growth.core`; remote reading projects the requested Experience from that remotely fetched backing Memory without creating a duplicate Experience Memory. `Belief`, `Context Split`, and `Grounded Action Instance` are decoded from their existing ordinary Memory records.
- Conflicting Belief evidence is returned unchanged (support/conflict/missing/observed facts all remain visible). The remote-read layer performs no conflict adjudication, reward scoring, semantic ranking or automatic local belief update; later Memory-owned executable structures may decide how to use the transient evidence.
- Existing Mesh grant, transport HMAC/TLS, shared-Memory authorization and capability validation are reused; no second mesh protocol, evidence database or remote cache service was added.
- Local gofmt, production compile harness, test compile harness, `go vet`, four-kind transient evidence behavior, disconnect-forgets behavior and conflict-preservation checks passed. Full repository recovery/gofmt/vet/test/race and CI PASS remain unreported until an actual repository runner executes them.
- Next: expose transient remote evidence as input to Memory-owned validation/branching without auto-import; remove the dead historical `routeCognition` helper if the restored source confirms no remaining caller; continue physical-kernel closeout.
